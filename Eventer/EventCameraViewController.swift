...
//  Eventer
//
//  Created by Grisha on 04/10/2015.
...
class EventCameraViewController: UIViewController {
    ...
    func setSubviews(){
        ...
        titleLabel.text = "Your Event Picture"
    ...
    func setCameraControls(){
        ...
        flipCameraButton.addTarget(self, action: #selector(flipCamera), forControlEvents: UIControlEvents.TouchUpInside)
...
//        let croppedImage = CameraManager.crop(self.imageView, toFrame: cropRect)
        ...
        let currentCameraInput = captureSession?.inputs[0] as! AVCaptureInput
        if (currentCameraInput as! AVCaptureDeviceInput).device.position == AVCaptureDevicePosition.Front {
            let rotatedImage = UIImage(CGImage: self.imageView.image!.CGImage! ,
                scale: 1.0 ,
